...
# Create panel data for max windspeeds

create_panels_max <- function() {
  ...
  for (year in 1980:2015) {
    wind_dat_adm1 <- vect(paste0("data/max_windspeed/max_windspeed_", year, "_adm1.shp")) %>%
      as.data.frame() %>% filter(GID_0 %in% iso_adm1) %>%
      select("GID_0", "NAME_0", "GID_1", "max_windsp")
    colnames(wind_dat_adm1) <- c("iso", "country", "gid", "max_windsp_knt")
    wind_dat_adm1$level <- "Adm1"
    
    wind_dat_adm2 <- vect(paste0("data/max_windspeed/max_windspeed_", year, "_adm2.shp")) %>%
      as.data.frame() %>% filter(GID_0 %in% iso_adm2) %>%
      select("GID_0", "NAME_0", "GID_2", "max_windsp")
    colnames(wind_dat_adm2) <- c("iso", "country", "gid", "max_windsp_knt")
    wind_dat_adm2$level <- "Adm2"
    
    wind_dat <- rbind(wind_dat_adm1, wind_dat_adm2)
    wind_dat$windsp_ms <- wind_dat$max_windsp * 0.514444
    wind_dat$year <- year
  ...
  # Save the panel data
  saveRDS(wind_dat_all, file="data/wind_dat_all_max.rds")
  ...
  for (iso3 in unique(wind_dat_all$iso)) {
    ...
    # Save the panel plot
    ggsave(paste0("data/panel_plots_max/", iso3, "_panel_max.jpeg"), plot  = plot,
