...
# Run ETWFE model with a unit-level linear trend to test PT assumption

run_etwfe_pt <- function(dat_all, adm_level, iso) {
  ...
  if (adm_level==2) {
    mod <- feols(married ~ cyclone2:i(cohort, i.year, ref=1, ref2=1980)/
                   (age_dm + rural_dm) +
