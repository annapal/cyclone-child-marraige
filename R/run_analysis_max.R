...
run_analysis_max <- function(all_dat, wind_dat_max) {
  # Get meta data
  ...
  # Add lagged variables to wind_dat_max (k=3)
  wind_dat2 <- wind_dat_max %>%
    arrange(gid, year) %>%
    group_by(gid) %>% 
    mutate(
  ...
  # Save merged data
  saveRDS(all_dat_merged, "data/all_dat_merged_max.rds")
  ...
  # Save the results
  write_xlsx(results_all, "results/results_all_max.xlsx")
  ...
  ggsave("figures/main_max.jpeg", plot = plot)
}
